...
export default function RegisterForm() {
    ...
    const registerValidationSchema = yup.object().shape({
        ...
        nombre_usuario: 
            yup.string().min(3).max(15).required("Campo obligatorio"),
        contraseña: 
            yup.string().min(3).max(15).required("Campo obligatorio"),
      })
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3004/clientes/clienteByUsername/${data.nombre_usuario}`, data);
            if (response.data !== null) {
              ...
              await axios.post("http://localhost:3004/clientes", data);
        ...
        <View style={styles.formContainer}>
            ...
            <View>
                {(errors.nombre_completo && touched.nombre_completo) &&
                    <Text style={{ fontSize: 10, color: 'red' }}>{errors.nombre_completo}</Text>
                }
                <TextInput
                style={styles.input}
                ...
                />
                {usuarioExiste && (
                    <Text style={{ fontSize: 10, color: 'red' }}>El usuario ya existe. Por favor, elija otro nombre de usuario.</Text>
                )}
                {(errors.nombre_usuario && touched.nombre_usuario) &&
                    <Text style={{ fontSize: 10, color: 'red' }}>{errors.nombre_usuario}</Text>
                }
                <TextInput
                style={styles.input}
                placeholder='Usuario'
